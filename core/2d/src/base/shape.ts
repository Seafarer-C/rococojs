...
/** 物体基类，有一些共同属性和方法 */
export class Shape extends EventCenter {
  /** 物体类型标识 */
  ...
  /** 绘制激活物体边框 */
  drawBorders(ctx: CanvasRenderingContext2D): Shape {
  ...
  /** 绘制包围盒模型的控制点 */
  drawControls(ctx: CanvasRenderingContext2D): Shape {
  ...
  translateToOriginPoint(
    ...
    return Util.rotatePoint(
      new Point(x, y),
      center,
      Util.degreesToRadians(this.angle)
    );
  }
  /** 转换成本地坐标 */
  ...
  /** 物体动画 */
  animate(props, animateOptions: IAnimationOption): Shape {
    let propsToAnimate = [];
  ...
  setActive(active: boolean = false): Shape {
    this.active = !!active;
    return this;
  }
  /**
  ...
  setPositionByOrigin(pos: Point, originX: string, originY: string) {
    ...
    let position = this.translateToOriginPoint(
      center,
      this.originX,
      this.originY
    );
  ...
  intersectsWithRect(selectionTL: Point, selectionBR: Point): boolean {
    ...
    let intersection = Intersection.intersectPolygonRectangle(
      [tl, tr, br, bl],
      selectionTL,
      selectionBR
    );
